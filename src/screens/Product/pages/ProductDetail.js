...
const ProductDetail = ({ route, navigation }) => {
  ...
  useEffect(() => {
    !productItem &&
      axios
        .get(`admin/api/2022-10/products/${id}.json`)
        .then((res) => {
      ...
      {productItem ? (
        <View style={{ width: "100%" }}>
          <View style={styles.imageSection}>
            <Image
              source={{ uri: productItem.images[selectedItem].src }}
              style={styles.image}
            />
          </View>
          <ScrollView
            style={[styles.subImageSection]}
            horizontal={true}
          ...
          <View style={styles.detailText}>
            ...
            <Text style={[styles.detailText, { color: "#555555" }]}>
              {convertHTMLtoString(productItem?.body_html)}
            </Text>
          </View>
          <View
            style={{
          ...
          >
            <View>
              <Text style={[styles.detailText, { color: "#DD8560" }]}>
                Price {productItem?.variants[selectedItem].price}
              </Text>
              <Text style={[styles.detailText, { color: "#555555" }]}>
                Color: {productItem?.variants[selectedItem].title}
              </Text>
            </View>
