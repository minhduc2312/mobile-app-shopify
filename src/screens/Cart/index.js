  ...
  return (
    cart.length ?
      (<View style={styles.container}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '100%', backgroundColor: '#fff', paddingHorizontal: 5, paddingVertical: 10 }}>
          {/* get quantity item in cart */}
          <Text style={{ fontSize: 16, fontWeight: '500' }}>Items ({cart.reduce((prev, curr) => {
        ...
        <TouchableOpacity style={{ width: Dimensions.get('screen').width - 20, alignItems: 'center', marginVertical: 10, paddingVertical: 10, backgroundColor: '#333' }}>
          <Text style={{ color: '#fff', fontSize: 18, fontWeight: '600' }}>PLACE ORDER</Text>
        </TouchableOpacity>
      </View>) : (
